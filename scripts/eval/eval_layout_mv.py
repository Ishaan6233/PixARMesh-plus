...
def decode_layout_tokens(
    tokens: np.ndarray, pos_token_offset: int, num_pos_tokens: int
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    raw_bins = tokens[:, :LAYOUT_COORD_TOKENS] - int(pos_token_offset)
    valid = (raw_bins >= 0) & (raw_bins < int(num_pos_tokens))
    ...
    return boxes.astype(np.float32), raw_bins.astype(np.int64), valid
...
def main() -> None:
    ...
    for batch in loader:
        ...
        pred_tokens = generated[:, :LAYOUT_COORD_TOKENS]
        pred_boxes, pred_bins, pred_valid = decode_layout_tokens(
            pred_tokens, model_cfg.pos_token_offset, model_cfg.num_pos_tokens
        )
        gt_boxes, gt_bins, _ = decode_layout_tokens(
            gt_tokens, model_cfg.pos_token_offset, model_cfg.num_pos_tokens
        )
        for i in range(pred_tokens.shape[0]):
            ...
            rec = {
                ...
                **bbox_metrics(
                ...
                "pred_tokens": pred_tokens[i].astype(int).tolist(),
