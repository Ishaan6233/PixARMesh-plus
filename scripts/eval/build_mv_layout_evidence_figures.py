...
from scripts.eval.summarize_mv_layout_evidence import HIGHER_IS_BETTER, LAYOUT_METRICS
...
def seed_records(layout_root: Path, run: str, seed: int) -> dict[str, dict[str, Any]]:
    return per_uid(
        read_jsonl(layout_seed_dir(layout_root, run, seed) / "per_sample.jsonl")
    )
    ...
    *,
    ...
    for seed in seeds:
        ...
        for uid in sorted(set(baseline) & set(candidate)):
            if metric not in baseline[uid] or metric not in candidate[uid]:
                continue
            score = metric_improvement(
                float(candidate[uid][metric]), float(baseline[uid][metric]), metric
            )
    ...
    for uid, values in by_uid.items():
        ranked.append(
            {
                "uid": uid,
                "mean_improvement": sum(item["improvement"] for item in values)
                / len(values),
                "seeds": values,
            }
        )
    ...
    *,
    ...
    for seed in seeds:
        ...
        for uid, record in sorted(candidate.items()):
            if not all(
                metric in record
                for metric in ("valid_token_frac", "aabb_iou", "bin_mae")
            ):
                continue
            failure_score = (
    ...
    for uid, values in by_uid.items():
        ranked.append(
            {
                "uid": uid,
                "mean_failure_score": sum(item["failure_score"] for item in values)
                / len(values),
                "seeds": values,
            }
        )
...
def write_union_uid_list(path: Path, groups: list[list[dict[str, Any]]]) -> list[str]:
    uids = sorted({str(item["uid"]) for group in groups for item in group})
    path.write_text("\n".join(uids) + ("\n" if uids else ""))
    return uids


def empty_uid_list_justifications(
    *,
    ...
    justifications = {}
    if not fixed:
        justifications["fixed"] = (
            f"No paired candidate/baseline UID had rank metric {rank_metric!r} available."
        )
    if not improved:
        if ranked:
            justifications["improved"] = (
                f"No ranked UID had positive mean {rank_metric!r} improvement over the baseline run."
            )
        else:
            justifications["improved"] = (
                f"No paired candidate/baseline UID had rank metric {rank_metric!r} available."
...
def plot_per_seed_metrics(summary: dict[str, Any], out_path: Path) -> None:
    ...
    for ax, metric in zip(axes.flatten(), LAYOUT_METRICS):
        for run, run_info in runs.items():
            xs = []
            ys = []
            for seed, seed_info in sorted(
                run_info.get("seeds", {}).items(), key=lambda item: int(item[0])
            ):
...
def build_figures(
    *,
    ...
    improved = [item for item in ranked if item["mean_improvement"] > 0][
        :max_gallery_cases
    ]
    regressed = list(
        reversed([item for item in ranked if item["mean_improvement"] < 0])
    )[:max_gallery_cases]
    ...
    gallery_uids = write_union_uid_list(
        out_dir / "gallery_uids.txt", [fixed, improved, regressed, failures]
    )
    justifications = empty_uid_list_justifications(
        fixed=fixed,
        improved=improved,
        ranked=ranked,
    ...
    manifest = {
        ...
        "uid_lists": {
            ...
            "gallery": str(out_dir / "gallery_uids.txt"),
        },
        "gallery_uids": gallery_uids,
        "empty_uid_list_justifications": justifications,
        "galleries": galleries,
        "created_composites": sum(len(gallery["created"]) for gallery in galleries),
    }
    (out_dir / "gallery_manifest.json").write_text(
