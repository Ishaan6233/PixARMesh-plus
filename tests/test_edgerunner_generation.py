...
class _DummyConfig:
    vocab_size = 20
    # Coordinate tokens span [6, 6+num_pos_tokens) = [6, 16) here — deliberately a
    # strict subset of vocab_size (20) so the grammar must NOT admit ids 16..19 as
    # coordinates (the bug this guards against).
    num_pos_tokens = 10
    eos_token_id = 2
...
class EdgeRunnerGrammarTest(unittest.TestCase):
    ...
    def test_bom_requires_nine_coordinate_tokens(self):
        self.assertEqual(self._allowed_for([5]), list(range(6, 16)))
        self.assertEqual(self._allowed_for([5] + [6] * 8), list(range(6, 16)))
        self.assertEqual(self._allowed_for([5] + [6] * 9), [3, 4, 5, 2])

    def test_lr_requires_three_coordinate_tokens(self):
        prefix = [5] + [6] * 9 + [3]
        self.assertEqual(self._allowed_for(prefix), list(range(6, 16)))
        self.assertEqual(self._allowed_for(prefix + [7, 8]), list(range(6, 16)))
