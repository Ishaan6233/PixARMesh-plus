...
def test_train_mv_wrapper_uses_strict_warm_start_for_stage2(tmp_path):
    log_path = tmp_path / "fake_python.log"
    stage1_root = tmp_path / "stage1"
    (stage1_root / "existing" / "checkpoints" / "final").mkdir(parents=True)
    fake_python = tmp_path / "python"
    fake_python.write_text(
        "#!/usr/bin/env bash\n"
        "printf 'ARGS=%s\\n' \"$*\" >> \"$FAKE_PYTHON_LOG\"\n"
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    log_text = log_path.read_text()
    assert "--config-name=edgerunner_3d_front_trellis2_mv_stage2" in log_text
    assert "model.local_path_load_mode=strict_warm_start" in log_text
