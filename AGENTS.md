...
Single source of shared guidance for **all** coding agents working in this repository — Codex
reads this file natively, and `CLAUDE.md` imports it for Claude Code. Edit this file; put
instructions in an agent's own file only when they are truly agent-specific.

Shared file-placement rules live in `docs/FILE_STRUCTURE.md`; read that before creating new
files, and place files by semantic purpose, not by whichever directory is currently open.
Generated `graphify/` and `graphify-out/` artifacts are optional navigation aids, not source of
truth, and should not be committed; verify claims against live code, configs, tests, and command
output.
...
```bash
# (no Makefile in this repo — run tools directly, in the pixarmesh124 env)
...
accelerate launch --module scripts.eval_obj --pred-dir <preds> --save-dir <out>
```
...
## Cross-agent workflow (Claude Code + Codex)

Both agents share this file as their project brief. Chat context does not transfer between them,
so all durable state must live in files and git:

- **Handoff protocol.** Before stopping work (rate limit, end of session), update
  `docs/HANDOFF.md` — current goal, decisions made and why, what is done, what is next, open
  gotchas — and commit WIP on the current branch with a descriptive message. An agent picking up
  work reads `docs/HANDOFF.md` plus `git log -5` / `git diff main` before doing anything else.
  In Claude Code the machine-local `/handoff` skill automates both directions (`/handoff` to
  write the doc + commit WIP, `/handoff in` to catch up on the gap).
- **Cross-review.** In Claude Code, `/codex-review` gets an independent adversarial review of the
  current diff from Codex and verifies its findings before reporting. From Codex, get Claude's
  opinion headlessly: `claude -p "<verification prompt>"`. Either direction: reviewers must give
  file:line evidence and a CONFIRM/REFUTE verdict per claim — never accept an unevidenced
  "looks good".
- **Tiebreaker rule.** When the two agents disagree, the resolution is a mechanical check (test,
  probe script, measurement) — not a third opinion.
