    ...
    if mask_q is None and mask_kv is None:
        if backend == "flash-attn" and FLASH_ATTN_AVAILABLE:
            return flash_attn_func(
                q, k, v, dropout, causal=causal, window_size=window_size
            )  # [B, N, H, D]
            ...
            if causal and N > 1:
                causal_mask = torch.full(
                    (N, M), float("-inf"), device=w.device, dtype=w.dtype
                )
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context, mask_q=None, mask_kv=None):
        ...
        x = attention(
            q, k, v, mask_q=mask_q, mask_kv=mask_kv, dropout=self.dropout, causal=False
        )  # [B, N, H, D]
        x = self.out_proj(x.reshape(B, N, -1))
        return x
