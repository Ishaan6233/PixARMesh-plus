...
def build_geo_ctx_pc(
    ...
    for b in range(B):
        ...
        if n_valid == 0:
            ...
            # The caller covers the frozen-geometry-unavailable case;
...
def _cap_points(pts: torch.Tensor, max_in: int) -> torch.Tensor:
    ...
    so FPS over the full ~1M-point merged geometry cloud is prohibitive (and far slower
...
def _sample_mask_ids(
    ...
    local_points_z: torch.Tensor,  # (N, H, W) frozen-geometry predicted depth
    ...
    # Sample predicted depth at projected positions
    d_flat  = local_points_z.float().unsqueeze(1)      # (N, 1, H, W)
    c_flat  = pix_coords.permute(1, 0, 2).unsqueeze(2) # (N, V, 1, 2)
    geo_d  = F.grid_sample(d_flat, c_flat, mode="bilinear",
                            padding_mode="zeros", align_corners=False)
    geo_d  = geo_d.squeeze(1).squeeze(-1).T          # (V, N) via (N, V, 1) -> squeeze

    depth_ok   = pts_cam[..., 2] <= geo_d * (1.0 + depth_rtol)
    in_bounds  = geo_d > 0
...
def _get_per_view_target_ids(
    ...
    K_per_view_n: torch.Tensor,        # (N, 3, 3)
    panoptic_masks: torch.Tensor,      # (N, H, W) long
    local_points_z: torch.Tensor,      # (N, H, W) frozen-geometry depth
    view_mask: torch.Tensor,           # (N,) bool
    depth_rtol: float = 0.10,
...
def _icp_align(src: torch.Tensor, tgt: torch.Tensor, w: torch.Tensor | None,
    ...
    reducing cross-view scale inconsistency (Fix A / CMVSA)."""
...
def discover_instance_points_mv(
    local_points: torch.Tensor,        # (B, N, H, W, 3)  per-view camera frame
    ...
    num_obj_voxels: int,
    num_ctx_voxels: int,
    conf: torch.Tensor | None = None,  # (B, N, H, W, 1) per-pixel confidence
    pool_size: int = 8192,
    depth_rtol: float = 0.10,
    ...
    For each frozen-geometry 3D point (from confidence-filtered adaptive FPS merged across N
    ...
    Args:
        ...
        conf:            (B, N, H, W, 1) per-pixel confidence, or None
    ...
    for b in range(B):
        ...
        # --- 1. Merge valid frozen-geometry points from all views to scene space ---
        pts_scene_list = []
        conf_list      = []
        for n in range(N):
            if not vm_b[n]:
        ...
        if mask_seeded_pool:
            ...
            for n in range(N):
                ...
                # local_points resolution (typically padded to size_divisor=28, e.g. 504x672).
            ...
            # Each list entry is one view's masked object surface in scene space; the
    ...
    for b in range(B):
        ...
        if reg_active:
            ...
            if adaptive_fallback and hit_count.max() < threshold:
                # Fallback: predicted depth maps may not be globally consistent across wide-
